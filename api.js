...
function Api() {
	...
	// Создать примечание к сделкe
	this.createNotes = authChecker((id_deal, data) => {
		return axios.post(
			`${ROOT_PATH}/api/v4/leads/${id_deal}/notes`,
			[].concat(data),
			{
				headers: {
					Authorization: `Bearer ${access_token}`,
	...
	// Получить задачи
	this.getTasks = authChecker((queryParam = "") => {
		return axios.get(`${ROOT_PATH}/api/v4/tasks?${queryParam}`, {
			headers: {
				Authorization: `Bearer ${access_token}`,
			},
		});
